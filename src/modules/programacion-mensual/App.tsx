...
const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
  ...
  return (
    <div 
      className={`fixed bottom-8 right-8 z-50 flex items-center p-4 pr-10 space-x-4 ${currentTheme.text} ${currentTheme.bg} rounded-xl shadow-2xl animate-fade-in-up transition-transform transform`} 
      role="alert"
    >
      ...
      <button
        type="button"
        className="absolute top-1 right-1 bg-transparent text-white rounded-lg focus:ring-2 focus:ring-white/50 p-1.5 hover:bg-white/20 inline-flex h-8 w-8"
        onClick={onClose}
        aria-label="Close"
      >
